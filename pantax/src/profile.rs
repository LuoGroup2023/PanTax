...
fn check_args_valid(args: &ProfilingConfig, input_file: &mut InputFile) {
    ...
    if args.strain {
        info!(
            "strain profiling main parameters: unique_trio_fraction: {}, strain_cov_diff: {}, single_cov_ratio: {}", 
            args.unique_trio_nodes_fraction, args.unique_trio_nodes_mean_count_f, args.single_cov_ratio
        )
    }
...
fn species_profiling(args: &ProfilingConfig, input_file: &InputFile, rcls_df: &DataFrame) -> PolarsResult<DataFrame> {
...
fn load_species_range(args: &ProfilingConfig, input_file: &InputFile, species_profile_df: &DataFrame) -> PolarsResult<Vec<SpeciesRange>> {
    let schema = Schema::from_iter(vec![
        Field::new("species".into(), DataType::String),
...
fn optimize_otu(args: &ProfilingConfig, otu: &String, start: u32, end: u32, reads_cluster: &[Record]) -> Option<Vec<HapMetrics>> {
...
fn abundance_est(args: &ProfilingConfig, hap_metrics_vec: &[HapMetrics]) -> Result<()> {
    let schema = Schema::from_iter(vec![
...
fn strain_profiling(args: &ProfilingConfig, input_file: &InputFile, species_profile_df: &DataFrame, rcls_df: &DataFrame) -> Result<()> {
...
pub fn profile(args: ProfilingConfig) -> Result<()> {
    ...
    if args.species && !file_exists(&input_file.species_abund_file) {
        ...
        let rcls_df = rcls::rcls_profile(&input_file)?;
