...
pub fn save_output_to_file(df: &mut DataFrame, output_file: &str, write_with_header: bool) -> PolarsResult<()> {
    CsvWriter::new(File::create(output_file)?)
        .with_separator(b'\t')
...
pub fn rcls(args: RclsArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.struct_read {
        ...
        //     }
        // }
        save_output_to_file(&mut output_df, &args.output_name.to_string_lossy(), false)?;
    } else {
        let species_info = load_species_range(&args.range_file)?;
